...
public class NoteViewHolder extends RecyclerView.ViewHolder {
    ...
    public void bind(final Note note) {
        ...
        int pri = note.getPriority();
        System.out.println("czy: " + "Context:" + note.getContent() + "\n pri:"+note.getPriority());

        if(pri == 3){
            itemview.setBackgroundColor(Color.rgb(255,0,0));
            //itemview.setAlpha(0.5f);
        }else if(pri == 2){
            itemview.setBackgroundColor(Color.rgb(0,250,0));

        }else{
            itemview.setBackgroundColor(Color.rgb(0,0,255));
        }
